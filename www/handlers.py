...
def authenticate(*, email, passwd):
    ...
    if user.passwd != sha1.hexdigest():
        raise APIValueError('passwd', 'Invalid password.')
    # authenticat ok, set cookie
    r = web.Response()
    r.set_cookie(COOKIE_NAME, user2cookie(user, 86400), max_age = 86400, httponly = True)
    user.passwd = '******'
    r.content_type = 'application/json'
    r.body = r.jumps(user, ensure_ascii = False).encode('utf-8')
    return r
...
def api_register_user(*, email, name, passwd):
    ...
    user = yield from User(id = uid, name = name.strip(), email = email, password = hashlib.sha1(sha1_passwd.encode('utf-8')).hexdigest(), image = 'http://net.120askimages.com/images/no_avatar.gif')
